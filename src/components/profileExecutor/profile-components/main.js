...
const Main = ({userProfile,matchedService}) => {
    ...
    if(userProfile.response.status === false) {
        window.location.replace('/')
    }
    let rateStart = userProfile && Number(userProfile.response.executorProfile.rate).toFixed(1)
    let photoStar
    ...
    const calculateAge = (dateString) => {
        ...
        const birthDate = userProfile.response.userProfile.birthday;
        ...
        <div className="profile-main">
            ...
            <div className="block-profile-main">
                <div className='profile-avatar'>
                    <img 
                        src={`http://194.67.113.55/` + userProfile.response.userProfile.avatar} 
                        alt='.' 
                        className='profile-img'
                    />
                    {userProfile.response.userProfile.online === false ? 
                ...
                <div className='block-profile-info'>
                    <div className='info-main'>
                        {userProfile.response.executorProfile.nameOrg && 
                        <div className='info-name'>
                            <span className='mr'>{userProfile.response.executorProfile.nameOrg}</span>
                        </div>
                        }
                        <div className='info-name-two'>
                            <h4 className='mr'>{userProfile.response.userProfile.name}</h4>
                            <h4>{userProfile.response.userProfile.surname}</h4>
                        </div>
                        <span className='info-user'>{matchedService && matchedService.name}</span>
                        <span className='map ml-minus'>
                               <img src={photo4} alt='.' width={24}/>
                                {userProfile.response.userProfile.country}, {userProfile.response.userProfile.city}
                            </span>
                        <span className='info-user mt'>{(age || age === 0) ? `${ageString}` : ''}   </span>
                        {userProfile.response.executorProfile.countRate ?
                            <>
                                 ...
                                 <span className='info-user'>{t('profile.profile-user.done')}: {userProfile.response.executorProfile.countRate} 
                                 {t('profile.profile-user.task')}{getDeclension(userProfile.response.executorProfile.countRate, taskArray)}</span>
                            </>
                            :
                            ''
                        }
