...
const ListUserCredit = () => {
    const [list, setList] = useState()
    const [search, setSearch] = useState('')
    useEffect(() => {
        getCreditRequests().then((data) => setList(data))
    },[])
    ...
    if (active === true) {
        listTwo = list && list.creditRequests && list.creditRequests.filter(res => res.needCredit !== 2)
    } else {
        listTwo = list && list.creditRequests && list.creditRequests
    }
    ...
    const handleButtonPostList = () => {
        getCreditRequests(search).then((data) => setList(data))
    }
      ...
      <div className="">
        ...
        <div className="none-mt container">
            <div className="work-search-block">
                <h1 className="work-search-text">Список Кредитных заявок</h1>
            ...
            <div className="block-adm-list">
                <ul>
                        <div className="block-list-user-all">
                            {list && list.creditRequests && listTwo.slice(0, displayCount).map((res,index) => {
                                return (
                                    <>
