...
const ListUserInsure = () => {
    const [list, setList] = useState()
    const [search, setSearch] = useState('')
    useEffect(() => {
        getInsuranceRequests().then((data) => setList(data))
    },[])
    ...
    if (active === true) {
        listTwo = list && list.insuranceRequests && list.insuranceRequests.filter(res => res.needInsurance !== 2)
    } else {
        listTwo = list && list.insuranceRequests && list.insuranceRequests
    }
    ...
    const handleButtonPostList = () => {
        getInsuranceRequests(search).then((data) => setList(data))
    }
      ...
      <div className="">
        ...
        <div className="none-mt container">
            <div className="work-search-block">
                <h1 className="work-search-text">Список Заявок на Страхование</h1>
        ...
        <div className="container">
            {active ?
            <button className="button-main-adm mt-15" onClick={() => setActive(false)}>Отобразить одобренные</button>
            :
            <button className="button-main-adm mt-15" onClick={() => setActive(true)}>Скрыть одобренные</button>
            }
            
            <div className="block-adm-list">
                <ul>
                        <div className="block-list-user-all">
                            {list && list.insuranceRequests && listTwo.slice(0, displayCount).map((res,index) => {
                                return (
                                    <>
                                    <div className="block-list-adm">
                                        ...
                                        <div className="block-list-user">
                                            ...
                                            <div className="block-list-card">
                                                <div className="block-list-user-about">
                                                    <li className="block-list-user-about-li">Юридическое название<br/> Заказчика</li> 
                                                </div>
